...
if iso == 0    
    ...
    [l,totC] = size(dQ);
    randVec = rand(totC,1);
    randVec = round(randVec * l,0);
    
    for nC = 1:totC
        dQ(:,nC) = circshift(dQ(:,nC),randVec(nC)); % circshift each cell's spike train by a random amount
    end
    ...
    dq_iso = dQ(:,ixCells([1:40 81:end]));
    mapping = compute_mapping(dq_iso, 'Isomap',2); % run the Isomap script
...
% Save your work
if iso == 1
    SaveAnalysis(pwd,'Isomap_WK_HdOnly_Shuffle',{Q; mapping; ixCells; tcReal; tcWk; tcRev; B; randVec},{'Q'; 'mapping'; 'ixEx'; 'hReal'; 'hWk'; 'hRev'; 'B'; 'randVec'});
end
...
set(gcf, 'Color','w')
 ...
 %% Figure 1: Isomap and ring quality (no color)

figure (1), clf
set(gcf, 'Color','w')

ax = scatter(mapping(:,1),mapping(:,2),80,[0.5 0.5 0.5],'filled');
%ax.MarkerFaceColor = col;
%ax.MarkerEdgeColor = 'none';
ax.SizeData = 10;
